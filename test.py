...
# --- 0. CONFIGURACIÓN INICIAL Y ESTÉTICA (¡AQUÍ ESTÁ EL CAMBIO PARA EL TEMA CLARO!) ---
...
QUESTIONS_PER_PAGE = 10 # Cambiado a 10 preguntas por página para 13 páginas
TOTAL_QUESTIONS = len(QUESTION_DATA) # Ahora es 130
...
def calculate_results():
    """Calcula las puntuaciones finales de los 5 rasgos."""
    results = defaultdict(lambda: {"score": 0, "count": 0})

    for q_id, answer in st.session_state['answers'].items():
        # Encontrar la pregunta en la lista (se puede optimizar si QUESTION_DATA fuera un diccionario)
        q_data = next((q for q in QUESTION_DATA if q[0] == q_id), None)
        
        if q_data:
            _, _, trait, reverse = q_data
            
            # Aplicar la inversión de puntaje si es necesario
            score = answer
            if reverse:
                # La escala de 5 puntos se invierte con (6 - respuesta)
                score = 6 - answer 
            
            results[trait]["score"] += score
            results[trait]["count"] += 1

    # Calcular el puntaje promedio y escalarlo de 0 a 100
    # La puntuación máxima es 5.0 (Totalmente de acuerdo en todos los ítems)
    # Factor de escalado: (Puntuación obtenida / Puntuación máxima posible) * 100
    # Donde Puntuación Máxima Posible = 5.0
    final_scores = {
        trait: ((data["score"] / data["count"]) - 1) / 4 * 100 if data["count"] > 0 else 0
        for trait, data in results.items()
    }
    
    # Redondear y limitar a 100 (aunque el cálculo lo hace por debajo de 100)
...
def display_test_page(page_index):
    ...
    for i, q_data in enumerate(questions_for_page):
        ...
        response = st.slider(
            ...
            min_value=1,       # Valor Mínimo Explícito (Totalmente en desacuerdo)
            max_value=5,       # Valor Máximo Explícito (Totalmente de acuerdo)
            value=current_value, # Valor inicial o guardado
            step=1,
            format="%d - %s",
            key=f"q_{q_id}",
...
def display_results_page(scores):
    ...
    # Interpretaciones más detalladas basadas en el nivel (Baja, Media, Alta)
    INTERPRETATION_DATA = {
        "O": {
